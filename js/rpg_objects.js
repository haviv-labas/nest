...
Game_Map.prototype.update_movable_block_positions = function() {

    this._movable_block_positions = []
    for (var i = 0; i < $gameMap._events.length; i++) {
        ...
        if ($gameMap._events[i]._characterName == "!Crystal")
        {
            this._movable_block_positions.push(`${$gameMap._events[i]._x},${$gameMap._events[i]._y}`)
        }

    }
    
    console.log("movable event locations _:")
    console.log(this._movable_block_positions)


};

Game_Map.prototype.update_movable_block_targets = function(t) {

    this._movable_block_targets = t

};


Game_Map.prototype.movable_block_positions_meet_targets = function() {

    if ($gameMap._movable_block_targets.length !== $gameMap._movable_block_positions.length )
    {
        return false
    }

    for (var i = 0; i < $gameMap._movable_block_targets.length; i++) 
    {

        if ($gameMap._movable_block_positions.includes($gameMap._movable_block_targets[i]))
        {
            continue
        }
        else 
